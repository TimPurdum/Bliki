namespace Markdig
// Imported from https://github.com/arthurrump/MarkdigExtensions/tree/master/src/MarkdigExtensions.SyntaxHighlighting
...
type MarkdownPipelineBuilderExtensions() =
    ...
    /// <summary>Highlight code in fenced code blocks</summary>
    /// <param name="pipeline">The Markdig <see cref="MarkdownPipelineBuilder"/> to add the extension to</param>
    static member UseFileLinkNewTab(pipeline : MarkdownPipelineBuilder) =
        pipeline.Extensions.Add(FileLinkNewTabExtension())
        pipeline
